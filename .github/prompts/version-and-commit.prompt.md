...
4. Ensure the readme is updated logically with any changes in design or functionality.
5. Choose the version bump using this policy unless the user supplied an explicit level or version:
...
6. If the user supplied `patch`, `minor`, `major`, or an exact semantic version, use that request instead of inferring the bump. Reject invalid or backward version changes.
7. Update the changelog before committing:
...
8. Update the package version consistently in `package.json` and `package-lock.json`. Prefer `npm version --no-git-tag-version <patch|minor|major>` for level bumps so npm updates both files without creating an automatic commit or tag. For an exact version, use the equivalent no-git-tag version update.
9. Run the repository validation commands from `package.json`, at minimum:
   - `npm test`
   - `npm run lint`
   - `npm pack --dry-run`
   - `git diff --check`
10. If validation fails, fix only release-related problems and rerun the failing check. Do not commit failing tests or lint errors.
11. Stage only the release-related files. Leave unrelated worktree changes unstaged and untouched.
12. Create exactly one concise Conventional Commit using a format such as:
    - `fix(tui): preserve history after process exit`
    - `feat(search): add history filtering`
    - `chore(release): prepare v0.3.0`
13. Do not add `Co-authored-by`, `Signed-off-by`, or any other author trailer. Do not use `--no-verify`.
14. Verify the final commit with `git show --stat --oneline HEAD`, `git show -s --format=%B HEAD`, and `git status --short --branch`.
